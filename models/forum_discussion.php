...
class Forum_Discussion_Model extends Auto_Modeler_ORM
{
	...
	{
		return $this->db->select('DISTINCT forum_discussions.*')->from('forum_discussions')->join('forum_comments', array('forum_comments.forum_discussion_id' => 'forum_discussions.id'))->limit($limit, ($page-1))->orderby('forum_comments.date')->get()->result(TRUE, 'Forum_Discussion_Model');
	}
}